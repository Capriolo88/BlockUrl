...
function deleteLists(name) {
    switch (name) {
        case 'malware':
            if (appMalware != null)
                appMalware.deleteAll(), appMalware = null;
            break;
        case 'phish':
            if (appPhish != null)
                appPhish.deleteAll(), appPhish = null;
            break;
        default :
            if (appMalware != null)
                appMalware.deleteAll(), appMalware = null;
    ...
    if (responseList != null)
        responseList.deleteAll(), responseList = null;
}
...
function downloadLists() {
    ...
    for (var key in responseList.lists) {
        var name = key.match(/(malware|phish)/)[0],
            id = setTimeout(updateList, l * 1000, name, responseList.lists[key], responseList.TTL);
...
//function createHashesBody(props) {
...
function requestFullHashes() {
    var xmlobj = createXMLHttp(), body;
