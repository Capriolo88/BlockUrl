    ...
    function handler() {
        if (httpobj.readyState === 4) {
            if (httpobj.status !== 200) {
                chrome.runtime.sendMessage({'command': 'uploadlist', 'status': 'request error', 'name': googlist.name});
                return;
            }
            console.log(redirecturls.length);
            var buffer = httpobj.response;
            if (!googlist.parseBuffer(buffer)) {
                chrome.runtime.sendMessage({'command': 'uploadlist', 'status': 'parse error'});
                return;
            }
            if (redirecturls.length > 0) {
                setTimeout(function () {
                ...
                chrome.runtime.sendMessage({'command': 'uploadlist', 'status': 'ok', 'name': googlist.name});
            }
        }
    }
}
